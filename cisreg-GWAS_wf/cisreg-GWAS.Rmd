...
varscan.jaspar.merged.signif <-  subset(varscan.jaspar.merged, pval_ratio >= 100)

ggplot(varscan.jaspar.merged.signif, aes(label, var_id)) + geom_tile(aes(fill = -log10(best_pval)),
     colour = "white") + 
     labs(x='Motifs', y = 'SNPs')+
      theme_bw()+
      guides(fill = guide_legend(title = "-log10(P-value)"))+
  ggtitle("Heatmap of rSNPs predicted by variation-scan (Pval-ratio > 100)") +
