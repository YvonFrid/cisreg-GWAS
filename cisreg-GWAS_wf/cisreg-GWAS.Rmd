---
#knit: (function(inputFile, encoding = "UTF-8") { rmarkdown::render(inputFile, encoding = encoding, output_file = 'cisreg_GWAS_report2.html', output_dir = "cisreg_GWAS_report_2019-08") })
...
## Draw a barplot with the number of SNPs by genomic context
## assign all the SNPs without documented consequence type to the "unknown" class
tagSNPsInfo$CONTEXT[tagSNPsInfo$CONTEXT == ""] <- "unknown"
...
```{r ld_snp_plot, fig.cap=paste(sep="", "**Figure ",(fig.nb <- fig.nb +1),". SNPs in linkage disequilibrium (LD).** "), out.width = "40%", fig.width=5, fig.height=8}
...
nb.SOI.remap <- length(unique(remap.SOI.overlaps$rsid))
workflowSummary["ReMap: co-localized SOIs with ChIP-seq peak"] <-
  nb.SOI.remap
...
# A subset of candidate rSNPs
candidate.rSNPs <- varscan.with.peaks[, c("seqnames", "var_id", "best_variant",  "worst_variant", "X.ac_motif", "remap.tf.name", "pval_ratio")]

write.table(x = candidate.rSNPs,
              file = outfiles["candidate rSNPs"],
              quote = FALSE,
              sep = "\t",
              row.names = FALSE,
...
```{r peak_annotation_rSNPs, fig.cap=paste(sep="", "**Figure ",(fig.nb <- fig.nb +1)," Annotation of the location of a given peak in terms of genomic features.** "), out.width = "80%", fig.width=8, fig.height=4.5}
...
if (nrow(rSNPs.info) == 0) {
  ...
  A <- ggplot(motif.alt.effect, aes(label, var_id)) + geom_tile(aes(fill = best_pval),
                                                                colour = "white") + scale_fill_gradient(low = "orange",
                                                                                                        high = "steelblue")+
    labs(x='', y = '')+
    theme_bw()+
    guides(fill = guide_legend(title = "-log10(P-value)"))+
...
if (nrow(varscan.with.peaks) == 0) {
  ...
  B <- ggplot(data=tissue, aes(x=Tissue, y=N, fill=name)) +
    ...
    theme_bw() +
    labs(x='tissue/cell', y = 'Frequency')+
    guides(fill = guide_legend(title = "TFs"))+
...
kable(as.data.frame(workflowSummary), 
 ...
 write.table(x = as.data.frame(workflowSummary))
 ...
 table1 <- as.data.frame(workflowSummary)
 
  write.table(x = table1, file = outfiles["GWAS-SNP associations (REST)"] , 
