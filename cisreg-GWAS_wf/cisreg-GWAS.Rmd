...
We used the `xEnricherSNPs` function (`XGR package` [@fang_xgr_2016]) for the enrichment analysis for SNPs using GWAS Catalog traits mapped to EFO.
...
#par(mfrow=c(2,2))  
# Barplot of motif
barplot(sort(table(jaspar.annotation.table$matrix.id), decreasing = TRUE),
...
```{r remapTF, fig.cap=paste(sep="", "**Figure ",(fig.nb <- fig.nb +1),". Transcription factors (TFs) enriched in ChIP-seq regions overlapping with the haplotype blocks of SNPs of interest (SOIs).** "), out.width = "80%", fig.width=12, fig.height=6}
