...
The piechart shows the genomic context of the `r length(DArsIDs)` SNPs associated to `r parameters$trait`.
...
message("Retrieving genomic information about ", length(DArsIDs), " disease-associated SNPs from dbSNP. ")
...
tagSNPsInfo$consequence_type_tv[tagSNPsInfo$consequence_type_tv == ""] <- "intergenic"
...
## Genomic features of SNPs (DA and LD SNPs)
...
snpInfo$cds_start <- is.na(snpInfo$cds_start)
SOIs <- subset(snpInfo, cds_start == TRUE)
#View(SOIs)
#dim(SOIs)
...
message("\tSelected ", nbSOIs, " SNPs of Interest (non-coding SNPs from GWAS or in LD)\t")
...
```{r  fig.cap = "Genomic context of SNPs of interest", out.width = "80%", fig.width=8, fig.height=4.5}
...
gene <- factor(SOIs$consequence_type_tv)
geneData <- as.data.frame(table(gene))

ggplot(geneData, aes(reorder(gene, Freq), Freq, fill = gene)) +
  geom_bar(stat = "identity", position = position_stack(), color = "black") +
  theme_classic() +
  #coord_flip() +
  #ggtitle("Genes and SNPs from HaploReg") +
  xlab("Contexte génomique") +
  ylab("SNPs") +
  guides(fill = FALSE) +
  coord_flip()
...
We identify the haplotype of a given SNP based on the pairs of LD SNPs previously retrieved from Ensembl as the limits of the region encompassing the SNPs in LD with this SNP.
...
variationScan <- read.delim(file = "~/cisreg-GWAS_results/EFO_0001068/RSAT/EFO_0001068_varscan_web.txt" ,
                            header = TRUE,
                            sep = "\t",
                            comment.char = ";")
...
nbvarScanJaspar <- nrow(varScanJaspar)
workflowSummary["variation-scan: potential rSNPs"] <- nbvarScanJaspar
```
...
if (file.exists(remapRdataFile)) {
  ...
  if (file.exists(remapBedFile)) {
    ...
    message ("Downloading the ReMap catalog")
    system.time({ download.file(parameters$remapUrl, dest = remapBedFile, method = "libcurl") })
  }
...
#remap.SOI.overlaps <- data.frame(IntersectBed(remapCatalog, haplotype.block.gr))
...
workflowSummary["remap.SOI.overlaps: overlaps between SOIs and ReMap peaks"] <- nbremap.SOI.overlaps
...
# Barplot of TFs
barplot(sort(table(remap.SOI.overlaps$name), decreasing = TRUE), 
        names.arg = "",
...
hist(sort(table(varscan.with.peaks$name), decreasing = TRUE), 
        names.arg = "",
        main  = "ReMap peaks overlapping \n SNPS of interest",
        xlab = "Ranked SNPs",
        ylab = "Overlapping peaks",
...
## Regulatory SNPs features

In this section, we presente the features of the potential regulatory SNPs.
...
## Genee
gene <- factor(rSNPs.data$hgnc_symbol)
gene.df <- as.data.frame(table(gene))

A <- ggplot(gene.df, aes(reorder(gene, Freq), Freq)) +
  geom_bar(stat = "identity", position = position_stack(), color = "black") +
  theme_classic() +
  #coord_flip() +
  #ggtitle("Genes and SNPs from HaploReg") +
...
TF <- factor(rSNPs.data$name)
TF.df <- as.data.frame(table(TF))

B <- ggplot(TF.df, aes(reorder(TF, Freq), Freq)) +
  geom_bar(stat = "identity", position = position_stack(), color = "black") +
  theme_classic() +
  #coord_flip() +
  #ggtitle("Genes and SNPs from HaploReg") +
...
# Merging the best_variant and wort_variant columns
rSNPs.data.mod <- rSNPs.data
rSNPs.data.mod$allele <- paste(rSNPs.data.mod$best_variant, 
                               rSNPs.data.mod$worst_variant, sep = "/")

allele <- factor(rSNPs.data.mod$allele)
allele.df <- as.data.frame(table(allele))

C <- ggplot(allele.df, aes(reorder(allele, Freq), Freq)) +
  geom_bar(stat = "identity", position = position_stack(), color = "black") +
  theme_classic() +
  #coord_flip() +
  #ggtitle("Genes and SNPs from HaploReg") +
...
context <- factor(rSNPs.data$consequence_type_tv)
context.df <- as.data.frame(table(context))

D <- ggplot(context.df, aes(reorder(context, Freq), Freq)) +
  geom_bar(stat = "identity", position = position_stack(), color = "black") +
  theme_classic() +
  #coord_flip() +
  #ggtitle("Genes and SNPs from HaploReg") +
