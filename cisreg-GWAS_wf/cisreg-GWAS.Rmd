...
DAsnpsSummary <- vector()

DAsnpsSummary["GWAS studies"] <- nbStudies

nbAss <- sum(redundancy.snps$N)
DAsnpsSummary["Association"] <- nbAss

redundancySNPs <- nrow(subset(redundancy.snps,
       N >= 2))

nrSNPs <- subset(redundancy.snps,
       N == 1)
nr.nbSNPs <- nrow(nrSNPs)

rsID <- as.vector(grep(pattern = '^rs\\d+', redundancy.snps$SNPS, perl = TRUE, value = TRUE))

nr.rsID <- intersect.Vector(rsID, nrSNPs$SNPS)
nr.rsID <- length(unique(nr.rsID))

DAsnpsSummary["SNPs redundant"] <- redundancySNPs
DAsnpsSummary["SNPs non-redundant"] <- nr.nbSNPs
DAsnpsSummary["SNPs non-redundant with rsID"] <- nr.rsID


kable(as.data.frame(DAsnpsSummary), caption = "Disease-associated SNPs")

#View(redundancy.snps)
...
variationScan <- read.delim(file = "~/cisreg-GWAS_results/EFO_0001360/RSAT/EFO_0001360_varscan_web.txt" ,
                            header = TRUE,
                            sep = "\t",
                            comment.char = ";")
...
nbMotis <- length(varScanJaspar$X.ac_motif)
message("Number of ref TFBM with at least one prediction in the SNPs: ", nbMotis)
workflowSummary["Motifs: Number of TFBM altered by the SOIs"] <- nbMotis
...
message("Prepare the TSS regions, which are defined as the flanking sequence of the TSS sites..")
...
We using `ChIPseeker` R package to annotate the ChIP-seq peak co-localised with the rSNPs. 
