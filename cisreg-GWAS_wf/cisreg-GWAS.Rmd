...
```{r DA_SNPs, fig.cap=paste(sep="", "**Figure ",(fig.nb <- fig.nb +1),". Genomic context of disease-associated variants from GWAS catalog.** "), out.width = "80%", fig.width=8, fig.height=4.5}
...
message("Gathering information from Ensembl BioMart for ", length(DArsIDs)," DA SNPs")
...
DA.snpInfo <- getBM(
  ...
  filters =  "snp_filter", 
  uniqueRows = TRUE,
  values = DArsIDs, # (DArsIDs + LDsnpIDs)
  mart = DA.snpmart)
...
message("Identified ", nb.valid.DA.snp.ID, 
        " rsIDs with information in Biomart among ", nbDiseaseAssociatedSNPs, " DA SNPs")
workflowSummary["valid rsIDs for BioMart"] <- nb.valid.DA.snp.ID

```

```{r da_snps_per_chrom_barplot, fig.cap="Number of Disease-associated SNPs per chromosome. ", out.width="50%", fig.width=6, fig.height=8}

## Drax a barplot with the number of disease-associated SNPs per chromosome
barplot(height = sort(table(DA.snpInfo$chr_name), decreasing = FALSE), 
        main = "Disease-associated SNPs per chromosome",
        horiz = TRUE, col = "#BBDDFF",
        cex.names = 0.8, las = 1)
...
rsIDs <- sort(unique(c(valid.DA.snp.ID, LDsnpIDs)))
nbSNPs <- length(rsIDs)
...
#         cex.lab = 1)
# 
ld.block <- sort(unlist(lapply(ldGroups, length)), decreasing = TRUE)
max.snps <- min(length(ld.block), 20)
y <- barplot(
  ld.block[1:max.snps],
  main = "LD block sizes",
  horiz = TRUE, 
  xlim = c(0, max(ld.block)*1.1),
  las = 1, 
  cex.names = 0.8,
  col = "#0D5F05",
  #main = "Number of SNPs in LD per SOI",
  xlab = "Number of SNPs in LD block",
  #ylab = "Disease-associated SNPs",
  cex.axis = 1,
  cex.lab = 1)
text(x = ld.block[1:max.snps], y = y, labels = ld.block[1:max.snps], pos = 4)
...
geneIDs.unique <- as.vector(sort(unique(snpInfo$ensembl_gene_stable_id)))
...
snpInfo.vs.geneInfo <- merge(x = snpInfo,
                             y = geneInfo.uniq,
...
```{r peak_annotation_rSNPs, results=FALSE}
