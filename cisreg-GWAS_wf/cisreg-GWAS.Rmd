...
for (i in 1:length(ldGroups)) {
  ...
  current.ldGroup.info <- data.frame(
    ...
    chr_name = SOIs[SOIs$refsnp_id == snp, "chr_name"],
    chrom_start = min(SOIs[SOIs$refsnp_id %in% ldGroup, "chrom_start"]),
    chrom_end = max(SOIs[SOIs$refsnp_id %in% ldGroup, "chrom_end"])
  )
