...
=======
  # query = "EFO_0001360", # type ii diabetes mellitus
>>>>>>> 2b1ae5e51a24d4119b08b43bc56f176d645c353c
...
dir.create(paste0(result.dir.path["JASPAR"],'pssm'), showWarnings = FALSE, recursive = TRUE)

api_root = "http://jaspar.genereg.net/api/v1/" ##Production server
#api_root = "http://127.0.0.1:8000/api/v1/" ##Local server

# Get information about JASPAR matrices, restricted to the core vertebrate collection 
url <- paste0(api_root,"matrix/?collection=CORE&tax_group=vertebrates&order=name&version=latest&page_size=10000&format=json")
result <- fromJSON(url)
print(result$count)
# View(result)
...
i <- 0
nbMatrices <- length(inter.matrix_ids)
message("Getting ", nbMatrices, " matrices from JASPAR REST web services")
for (matrix_id in inter.matrix_ids) {
  i <- i + 1
  ## Define the path of the local file  
  pssm_file <- file.path(result.dir.path["JASPAR"], paste0(matrix_id, ".txt"))
  
  if (file.exists(pssm_file)) {
    message("\tmatrix ", i, "/", nbMatrices, "\t", matrix_id, "\tfile exists\t", pssm_file)
  } else {
    ...
    message("\tmatrix\t", i, "/", nbMatrices, "\t", matrix_id, "\tdonwloaded from ", matrix_url)
    ...
    results_matrix = rbind(
      ...
      c(matrix_id, 
        matrix_result$name, 
        paste(matrix_result$uniprot_ids, collapse = ","), 
        jaspar2018_pubmedid, paste(matrix_result$pubmed_ids, collapse = ","), 
        source, 
        matrix_result$type))
    
    #Save raw  
    #pssm_file = paste0(result.dir.path["JASPAR"],'pssm/',matrix_result$matrix_id,".txt")
    
    ## JvH 2019-07-26: YVON, CECI ME SEMBLE INCORRECT, LES MATRICES QUE TU EXPORTES SONT TRANSPOSEES PAR RAPPORT AU FORMAT JASPAR. 
    ### LES UTILISES-TU VRAIMENT ? SINON IL FAUDRAIT SUPPRIMER CETTE SECTION QUI PREND UN TEMPS ENORME POUR RIEN
    cat(paste0(">",matrix_id, " ", matrix_result$name,"\n"), file = pssm_file)
    write.table(t(as.data.frame.list(matrix_result$pfm[c("A", "C", "G","T")])), 
                pssm_file, sep = "\t", 
                col.names = FALSE, 
                row.names = FALSE, 
                append = TRUE)
    message("\tmatrix ", i, "/", nbMatrices, "\t", matrix_id, "\tsaved to file\t", pssm_file)
  }
}
...
write.table(jaspar_results, paste0(result.dir.path["JASPAR"],"jaspar2018_annotations.tsv"), sep="\t", col.names = TRUE, row.names = FALSE, quote = FALSE)
