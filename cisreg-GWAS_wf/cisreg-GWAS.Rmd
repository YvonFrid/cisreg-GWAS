...
# Create a data frame
allSNPs.Context <- snpInfo$consequence_type_tv
allSNPs.DA.df <- as.data.frame.table(sort(table(allSNPs.Context), decreasing = T))
## Make the frequencies numbers (rather than factors)
## JvH (2019-07-25): this is not necessary with as.data.frame.table() above
# DA.df$Freq <- unlist(DA.df$Freq)

# Barplot of genomic context of DA-SNPs
A <- ggplot(data = allSNPs.DA.df, aes(x = allSNPs.Context, y = Freq)) +
  geom_bar(stat = "identity", fill = "#207DC2", color = "black") +
...
# Create a data frame
SOIs.Context <- SOIs$consequence_type_tv
SOIs.DA.df <- data.frame(sort(table(SOIs.Context), decreasing = T))

B <- ggplot(data = SOIs.DA.df, aes(x = SOIs.Context, y = Freq)) +
  geom_bar(stat = "identity", fill = "#207DC2", color = "black") +
  geom_text(aes(label = Freq), vjust = -0.1, hjust = -0.5, size = 3.5) +
  xlab("Genomic context") +
  ylab("Frequency") +
