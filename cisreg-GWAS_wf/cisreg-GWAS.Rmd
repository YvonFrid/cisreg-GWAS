...
#parameter.file <- "parameters/type2_diabetes.yaml"
#parameter.file <- "parameters/tuberculosis.yaml"
#parameter.file <- "parameters/diabetic_neuphropathy.yaml"
#parameter.file <- "parameters/sepsis.yaml"
#parameter.file <- "parameters/schizophrenia.yaml"
...
## Note: we temporarily disable ReMapEnrich because it contains system-wise dependencies, 
## which require admin rights to be installed.
LoadLibraries(auto.install = TRUE, github.libraries.to.install = NULL)
...
```{r manhattan_plot, fig.cap=paste(sep="", "**Figure ", (fig.nb <- fig.nb +1), ". Manhattan plot of the significance SNPs from genome-wide association study (GWAS)**. The X-axis shows the chromosomes. The Y-axis represente the negative logarithm of the association P-value for each SNP. Each dot on the Manhattan plot signifies a SNP. The strongest associations have the smallest P-values"), out.width = "100%", fig.width=12, fig.height=8}



#par(mfrow = c(1,1)) 

## Ckecking and select the valid SNPs rsID from GWAS catalog
check.valid.snps <- merge(x = studiesPerSNP,
                    y = DA.snpInfo,
                    by.x = "SNP",
                    by.y = "refsnp_id",
                    all.x = FALSE,
...
## Merging the valid SNP table by the GWAS results table by out invalid SNPs
valid.snps <- merge(x = check.valid.snps,
                       y = gwas.results,
                       by.x = "SNP",
                       by.y = "SNPS",
                       all.x = TRUE,
...
## Remove the duplicated rsID
valid.snps <- valid.snps[!duplicated(valid.snps$SNP),]

#View(valid.snps)
#dim(valid.snps)

## Select a subset data for manhattan plot
gwas.manhattan <- data.frame(SNP = valid.snps$SNP,
                             CHR = valid.snps$chr_name,
                             P = valid.snps$P.VALUE)
gwas.manhattan$BP <-  seq(valid.snps$SNP)
...
```{r da_snps_barplot, fig.cap=paste(sep="", "**Figure ",(fig.nb <- fig.nb +1),". Disease-associated SNPs (DA-SNPs) features**. (A) Genes associated with DA-SNPs. (B) Genomic context of disease-associated SNPs from GWAS catalog."), out.width="100%", fig.width=8, fig.height=6}

par(mfrow = c(1,2)) 
df <- sort(table(valid.snps$REPORTED.GENE.S.),  decreasing = FALSE)
...
df <- sort(table(valid.snps$CONTEXT),  decreasing = FALSE)
...
x <- barplot(height = df,
        ...
        cex.lab = 1,
        horiz = TRUE, 
        col = rainbow(length(unique(valid.snps$CONTEXT))),
        cex.names = 1, 
        las = 1)
...
x <- barplot(height = ld.df,
        ...
        cex.names = 1, 
        las = 1)
text(x = ld.df[1:max.context], y = x, labels = ld.df[1:max.context], cex = 1, pos = 4)
...
x <- barplot(height = soi.df,
        ...
        cex.names = 1, 
        las = 1)
text(x = soi.df[1:max.context], y = x, labels = soi.df[1:max.context], cex = 1, pos = 4)
#box()
```
...
We compute the coordinates of each LD groups
...
y <- barplot(
  ...
  horiz = TRUE, 
  xlim = c(0, max(snps.per.ldblock)*1.5),
  las = 1, 
  cex.names = 1,
  col = "#0D5F05",
  #main = "Number of SNPs in LD per SOI",
...
ggplot(varscan.jaspar.merged, aes(name, var_id)) + geom_tile(aes(fill = -log10(best_pval)),
     colour = "white") + 
...
#### Plot the distributions of P-value ####
...
best.sig <- -log10(varscan.jaspar.merged$best_pval)
worst.sig <- -log10(varscan.jaspar.merged$worst_pval)
sig.range <- range(c(best.sig, worst.sig))

n <- nrow(varscan.jaspar.merged)
f <- 1:n / n
plot(sort(best.sig, decreasing = TRUE), f, type = "l", col = "blue", 
     las = 1,
     xlim = sig.range,
     main = "A. P-value distributions",
     xlab = "-log10(P-value)",
     ylab = "Frequency", 
     panel.first = grid(col = "#DDDDDD", lty = "solid"))
lines(sort(worst.sig, decreasing = TRUE), f, type = "l", col = "red")
legend("topright", legend = c("Best P-value", "Worst P-value"),
...
#### Distribution of best and worst weights ####
...
nbremap.SOI <- length(unique(remap.SOI.overlaps$rsid))
workflowSummary["ReMap: number of SNPs with ReMap peaks"] <- nbremap.SOI
...
## Functional enrichment analysis of genes associated with the SOIs

We performe the functional enrichment analysis to identify predominant biological themes among the genes associated with the SOIs (Gene Of Interest, GOI) by incorporating biological knowledge provided by biological ontologies.
...
```{r functional_analysis_rSNPs, results=FALSE, fig.cap=paste(sep="", "**Figure ",(fig.nb <- fig.nb +1),". Functional enrichment analysis of the gene in reactome biological pathway. This is a subset of most enrichissed terms. The p.adjust represent the enriched score and the number of gene by dot size.** "), out.width = "80%", fig.width=8, fig.height=4.5}
  ...
  # SOI.gene.bed <- SOIs.annotations[, c("chromosome_name", "start_position", "end_position", "hgnc_symbol")]
  # 
  # SOI.gene.bed <- na.omit(SOI.gene.bed)
  # SOI.gene.bed <- SOI.gene.bed[!duplicated(SOI.gene.bed$start_position),]
  # 
  # #View(SOI.gene.bed)
  # 
  # SOI.gene.bed.gr <- with(SOI.gene.bed,
  ...
  # 
  # 
  # geneAnno <- annotatePeak(SOI.gene.bed.gr, #tssRegion=c(-3000, 3000),
  #                          TxDb=txdb, annoDb = "org.Hs.eg.db")
  # 
  # #print(geneAnno)
  # 
  # result <- try({
  #   gene <- seq2gene(SOI.gene.bed.gr, 
...
```{r gene_enrich, results=FALSE, out.width = "80%", fig.width=8, fig.height=4.5, fig.cap=paste(sep="", "**Figure ",(fig.nb <- fig.nb +1),". Top 30 significant enriched terms of functional enrichment of the genes of interest (GOI) using enrichment gene-disease associations of DisGeNET (Janet et al. 2015)**. Each horizontal row corresponds to one enriched term. The adjusted P-value represents the enrichment scores and gene count or ratio as circle and color.")}

#### Gene enrichment analysis with DOSE::DisGeNET ####

  # Select the GOI
  geneList <- na.omit(SOIs.annotations$entrezgene_id)
  nbGeneList <- length(geneList)
  geneList <- sort(unique(geneList))
    ...
    ## Doplot of enrichment gene-disease associations from DisGeNET
    dotplot(edo, showCategory=20) + ggtitle("Dotplot for over representation analysis")
...
### Exploring tissue-specific gene enrichment associated with the SOI

#### Tissue-specific gene enrichment bar chart

The below bar charts shown the tissue-specific gene enrichment.

```{r tissue_specific_gene_enrichment, results=FALSE, out.width = "80%", fig.width=8, fig.height=4.5, fig.cap=paste0("**Figure ",(fig.nb <- fig.nb +1),". Tissue-specific gene enrichment.** The x-axis shows each of the tissues, and the y-axis represents the tissue-specific gene enrichment values. The plot shows the visualization tissue-specific gene enrichment using the −Log10(P−Value) (up) and the fold-change values (below).")}

message("Tissue-specific gene enrichment analysis")

if (nrow(SOIs.annotations) == 0) {
  message("The annotation of the SOIs did not return a single predicted gene. Skipping tissue-specific gene enrichment. ")
    
  } else {
    
  ## Select the tenes associated with the rSNPs
  inputGenes <- unique(na.omit(SOIs.annotations$hgnc_symbol))
...
ggplot(enrichmentOutput,
    ...
    # B <- ggplot(enrichmentOutput,
    #        aes(x=reorder(Tissue,-fold.change),
    #            y=fold.change,label = Tissue.Specific.Genes,
    #            fill = Tissue))+
    #   geom_bar(stat = 'identity')+
...
#### Heatmap to show expression profile of tissue-specific genes

In this section, we vizualise in the form of a heatmap the expression values of the tissue-specific genes identified from the input gene set.


```{r expression_value, results=FALSE, out.width = "80%", fig.width=8, fig.height=4.5, fig.cap=paste0("**Figure ",(fig.nb <- fig.nb +1),". Expression profile of tissue-specific genes.** The x-axis shows each of the tissues, and the y-axis represents the tissue-specific gene expression.")}
seExp <- output[[2]][["Cerebral Cortex"]]
...
ggplot(exp, aes(key, Gene)) + geom_tile(aes(fill = value),
     colour = "white") + scale_fill_gradient(low = "steelblue",
     high = "red")+
     labs(x='', y = '')+
      theme_bw()+
...
```{r plot_remap_vs_varscan}
## Plotting the number of TFs founded by remap and by variation-scan



plot(SOIs.annotations[, c("ReMap.nb.TFs" ,"varscan.TF.nb")], 
     pch = 20, 
     bty="o",
     cex.axis = 1,
     #cex.main = 1,
     cex.lab = 1.5,
     col = c("blue", "red"),
     main = "Distribution of transcription factors (TFs)",
     xlab = "TFs from ReMap",
     ylab = "TFs from variation-scan",
     panel.first = grid(col = "#DDDDDD", lty = "solid"))
legend("topright", legend = c("ReMap", "variation-scan"), cex = 1, col = c("blue", "red"), pch = 20)


df1 <- as.matrix(SOIs.annotations[, c("ReMap.nb.TFs" ,"varscan.TF.nb", "remap.inter.varscan.nb.TFs")])

df <- as.matrix(data.frame("variation-scan" = sum(SOIs.annotations$varscan.TF.nb),
                           ReMap.vs.varscan = sum(SOIs.annotations$remap.inter.varscan.nb.TFs),
                           ReMap = sum(SOIs.annotations$ReMap.nb.TFs)
...
```{r peak_annotation_rSNPs, results=FALSE, fig.cap=paste(sep="", "**Figure ",(fig.nb <- fig.nb +1),". Pieplot of genomic annotation. Annotation of the location of a given peak in terms of genomic features.** "), out.width = "80%", fig.width=8, fig.height=4.5, warning=FALSE}
...
if (nrow(rSNPs.info) == 0) {
  ...
  ## rSNPs with highest effect of motif alteration
  motif.alt.effect <- varscan.with.peaks %>% 
    dplyr::select(var_id, label, best_pval) 
  #dplyr::group_by() %>% 
  #dplyr::summarise(N = n()) 
  motif.alt.effect$best_sig = -log10(motif.alt.effect$best_pval)
  
  
  A <- ggplot(motif.alt.effect, aes(label, var_id)) + geom_tile(aes(fill = best_sig),
...
if (nrow(varscan.with.peaks) == 0) {
  ...
  A <- ggplot(snp.tf, aes(name, var_id)) + 
  ...
  C <- ggplot(gene.ass, aes(hgnc_symbol, var_id)) + 
    geom_bar(stat="identity", position=position_dodge(), color="black") +
    #geom_tile(aes(fill = hgnc_symbol)) +
