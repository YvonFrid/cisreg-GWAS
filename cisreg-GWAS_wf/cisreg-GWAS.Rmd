...
if (is.null(parameters$remap$BedFile)) {
  parameters$remap$parameters$remap$remap$RdataFile <- 
...
if (parameters$RSAT$rest) {
  ...
  # View(varInfo)
  message("\tElapsed time to get variation info from RSAT REST: ", 
...
if (file.exists(parameters$remap$remap$RdataFile)) {
  message("Loading ReMap peaks from Rdata file.")
  # Load the ReMap Rdata file
  system.time({load(parameters$remap$remap$RdataFile)})
} else {
  ...
  system.time({remap.data <- fread(file = parameters$remap$BedFile) })
  ...
  system.time({  save(remapCatalog.gr, file = parameters$remap$remap$RdataFile) })
}
...
if (nrow(rSNPs.info) == 0) {
  ...
  } else if (rSNPS.nb.genes < parameters$min.genes) {
    message("The number of genes associated to the candidate rSNPs (", rSNPS.nb.genes, ")",
    " is lower than the user-specified threshold (", parameters$min.genes, "). ", 
